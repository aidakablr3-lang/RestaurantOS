"""ReopenBranchUseCase -- SS7's ``POST /api/v1/branches/{id}/reopen``
(``temporarily_closed -> active`` via ``Branch.reopen()``'s own
guard)."""
...
from restaurant_os_api.modules.restaurant.domain.events import BranchReopened
...
class ReopenBranchUseCase:
    ...
    async def execute(self, tenant_id: str, branch_id: str) -> BranchDTO:
        ...
        async with UnitOfWork(self._session_factory, TenantContext(tenant_id)) as uow:
            ...
            await outbox.publish(tenant_id, BranchReopened(branch_id=branch.id, occurred_at=now))
