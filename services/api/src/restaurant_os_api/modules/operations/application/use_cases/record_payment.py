...
payment's *non-tip* portion, plus this payment's own non-tip portion,
against the bill's ``amount_due`` (order subtotal + tax + adjustments).
Tips are deliberately excluded from that comparison -- a guest paying
a tip on top of the bill is not "overpaying the bill," it's a separate
addition ``TIPS_PAYABLE`` accounts for on its own.

**Ledger posting (Data Architecture v2.0 Group I):** one debit against
``CASH``/``CARD_CLEARING`` (by ``tender_type``) for the full payment
amount, matched by credits to ``SALES_REVENUE``, ``SALES_TAX_PAYABLE``,
and (if any) ``TIPS_PAYABLE`` -- split proportionally from the order's
own subtotal/tax ratio. **Disclosed limitation:** this proportional
split uses the *order's* totals, not a Tab's combined total across
multiple orders -- correct for the common single-order-bill path this
step's own tests exercise; Tab-based bills (out of scope for
...
and the underlying Order closes with it (``Order.close()``) -- "closing
out a table" in real POS terms. Publishes ``PaymentSettled``.
"""
...
class RecordPaymentUseCase:
    ...
    async def execute(
        ...
        async with UnitOfWork(self._session_factory, TenantContext(tenant_id)) as uow:
            ...
            prior_bill_paid = sum(
                (
                    p.amount - p.tip_amount
                    for p in existing_payments
                    if p.status == PaymentStatus.SETTLED
                ),
                Decimal(0),
            )
            this_bill_portion = request.amount - request.tip_amount
            new_total_paid = prior_bill_paid + this_bill_portion
            ...
            if revenue_tax_basis > 0:
                tax_share = (this_bill_portion * tax / revenue_tax_basis).quantize(
                    Decimal("0.0001")
                )
            else:
                tax_share = Decimal(0)
            revenue_share = this_bill_portion - tax_share
