"""End-to-end HTTP tests for ModifierGroup CRUD against a real
PostgreSQL instance (Sprint 5 Step 4.9).

Follows test_restaurant_router.py's exact pattern -- tenant-wide
authorization, no branch dimension, no parent to nest under (unlike
test_menu_category_router.py's restaurant_id nesting).
"""
...
async def owner(session_factory, client: TestClient) -> AsyncGenerator[dict]:
    """A user holding menu.read/menu.manage tenant-wide."""
...
def _create_body(**overrides) -> dict:
    body = {"name": "Size", "selectionType": "single"}
    body.update(overrides)
    return body


def _create_modifier_group(client: TestClient, owner: dict, **overrides) -> dict:
    response = client.post(
...
class TestCreateModifierGroup:
    def test_a_manage_holder_can_create_a_modifier_group(
        self, client: TestClient, owner: dict
    ) -> None:
        response = client.post(
            "/api/v1/modifier-groups", headers=_auth_headers(owner["token"]), json=_create_body()
        )
        assert response.status_code == 201, response.text
        data = response.json()["data"]
        assert data["name"] == "Size"
        assert data["selectionType"] == "single"
        assert data["tenantId"] == owner["tenant_id"]

    def test_a_duplicate_name_is_allowed(self, client: TestClient, owner: dict) -> None:
        _create_modifier_group(client, owner, name="Size")
        response = client.post(
            "/api/v1/modifier-groups",
            headers=_auth_headers(owner["token"]),
            json=_create_body(name="Size"),
        )
        assert response.status_code == 201, response.text

    def test_requires_authentication(self, client: TestClient) -> None:
        response = client.post("/api/v1/modifier-groups", json=_create_body())
        assert response.status_code == 401

    def test_denied_without_menu_manage(self, client: TestClient, reader_only: dict) -> None:
        response = client.post(
            "/api/v1/modifier-groups",
    ...
    async def test_denied_when_the_only_grant_is_on_an_inactive_role(
        ...
        response = client.post(
            "/api/v1/modifier-groups", headers=_auth_headers(token), json=_create_body()
        )
        assert response.status_code == 403
    ...
    async def test_denied_once_the_grant_is_revoked(
        ...
        token = _login_sync(client, tenant_id=owner["tenant_id"], email=email)
        first = client.post(
            "/api/v1/modifier-groups", headers=_auth_headers(token), json=_create_body(name="First")
        )
    ...
    def test_missing_required_field_is_rejected(self, client: TestClient, owner: dict) -> None:
        response = client.post(
            "/api/v1/modifier-groups", headers=_auth_headers(owner["token"]), json={}
        )
        assert response.status_code == 422

    def test_an_invalid_selection_type_is_rejected(self, client: TestClient, owner: dict) -> None:
        response = client.post(
            "/api/v1/modifier-groups",
            headers=_auth_headers(owner["token"]),
            json=_create_body(selectionType="not-a-real-type"),
        )
        assert response.status_code == 422


class TestCreateModifierGroupIdempotency:
    def test_the_same_key_and_body_replays_the_original_response(
        ...
        first = client.post("/api/v1/modifier-groups", headers=headers, json=_create_body())
        second = client.post("/api/v1/modifier-groups", headers=headers, json=_create_body())

        assert first.status_code == second.status_code == 201
        assert first.json() == second.json()

        list_response = client.get("/api/v1/modifier-groups", headers=_auth_headers(owner["token"]))
    ...
    def test_the_same_key_with_a_different_body_is_a_dedicated_conflict(
        ...
        first = client.post(
            "/api/v1/modifier-groups", headers=headers, json=_create_body(name="First")
        )
        assert first.status_code == 201, first.text

        second = client.post(
            "/api/v1/modifier-groups", headers=headers, json=_create_body(name="Second")
        )
...
class TestGetModifierGroup:
    def test_a_read_holder_can_get_a_modifier_group(self, client: TestClient, owner: dict) -> None:
        modifier_group = _create_modifier_group(client, owner)

        response = client.get(
            f"/api/v1/modifier-groups/{modifier_group['id']}",
            headers=_auth_headers(owner["token"]),
        )
        assert response.status_code == 200, response.text
        assert response.json()["data"]["id"] == modifier_group["id"]

    def test_an_unknown_id_is_404(self, client: TestClient, owner: dict) -> None:
        response = client.get(
            f"/api/v1/modifier-groups/{'0' * 26}", headers=_auth_headers(owner["token"])
        )
        assert response.status_code == 404
        assert response.json()["error"]["code"] == "MODIFIER_GROUP_NOT_FOUND"

    async def test_a_modifier_group_in_another_tenant_is_404_not_a_leak(
        self, client: TestClient, owner: dict, session_factory
    ) -> None:
        ...
        other_group = client.post(
            "/api/v1/modifier-groups", headers=_auth_headers(other_token), json=_create_body()
        ).json()["data"]

        response = client.get(
            f"/api/v1/modifier-groups/{other_group['id']}", headers=_auth_headers(owner["token"])
        )
        assert response.status_code == 404
        assert response.json()["error"]["code"] == "MODIFIER_GROUP_NOT_FOUND"

    def test_requires_authentication(self, client: TestClient) -> None:
        response = client.get(f"/api/v1/modifier-groups/{'0' * 26}")
        assert response.status_code == 401

    def test_denied_with_no_permission(self, client: TestClient, no_permission: dict) -> None:
        response = client.get(
            f"/api/v1/modifier-groups/{'0' * 26}", headers=_auth_headers(no_permission["token"])
        )
        assert response.status_code == 403
...
class TestListModifierGroups:
    def test_lists_only_the_callers_own_tenant(
        ...
        for name in ("A", "B", "C"):
            _create_modifier_group(client, owner, name=name)

        response = client.get("/api/v1/modifier-groups", headers=_auth_headers(owner["token"]))
        assert response.status_code == 200
        assert response.json()["meta"]["total"] == 3

    def test_pagination_offset_and_limit(self, client: TestClient, owner: dict) -> None:
        for i in range(5):
            _create_modifier_group(client, owner, name=f"G{i}")

        page_1 = client.get(
            "/api/v1/modifier-groups?offset=0&limit=2", headers=_auth_headers(owner["token"])
        )
        page_2 = client.get(
            "/api/v1/modifier-groups?offset=2&limit=2", headers=_auth_headers(owner["token"])
        )
        ...
        ids_page_1 = {g["id"] for g in page_1.json()["data"]}
        ids_page_2 = {g["id"] for g in page_2.json()["data"]}
        assert ids_page_1.isdisjoint(ids_page_2)

    def test_requires_authentication(self, client: TestClient) -> None:
        assert client.get("/api/v1/modifier-groups").status_code == 401

    def test_reader_only_can_list(self, client: TestClient, reader_only: dict) -> None:
        response = client.get(
            "/api/v1/modifier-groups", headers=_auth_headers(reader_only["token"])
        )
        assert response.status_code == 200

    def test_denied_with_no_permission(self, client: TestClient, no_permission: dict) -> None:
        response = client.get(
            "/api/v1/modifier-groups", headers=_auth_headers(no_permission["token"])
        )
        assert response.status_code == 403


class TestUpdateModifierGroup:
    def test_a_manage_holder_can_update_a_modifier_group(
        self, client: TestClient, owner: dict
    ) -> None:
        modifier_group = _create_modifier_group(client, owner)
        ...
        response = client.patch(
            f"/api/v1/modifier-groups/{modifier_group['id']}",
            headers=_auth_headers(owner["token"]),
            json={"name": "Renamed", "selectionType": "multiple"},
        )
        assert response.status_code == 200, response.text
        data = response.json()["data"]
        assert data["name"] == "Renamed"
        assert data["selectionType"] == "multiple"
        assert data["id"] == modifier_group["id"]

    def test_updating_an_unknown_id_returns_404(self, client: TestClient, owner: dict) -> None:
        response = client.patch(
            f"/api/v1/modifier-groups/{'0' * 26}",
            headers=_auth_headers(owner["token"]),
            json=_create_body(),
        )
        assert response.status_code == 404
    ...
    def test_idempotent_update_replays_on_the_same_key_and_body(
        ...
        url = f"/api/v1/modifier-groups/{modifier_group['id']}"
