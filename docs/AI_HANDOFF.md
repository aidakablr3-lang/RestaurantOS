...
**Updated by:** Sprint 5 Step 1 (Restaurant Platform architecture planning, documentation-only), Step 2 (RBAC Foundation: architecture doc + 8-commit implementation, plus a user-directed **Commit 9 correction pass** fixing a real router-level branch-scoped-authorization gap), Step 3 (**Restaurant Platform Data Layer**: 15-table migration `0004`, domain model, repositories, and a real-PostgreSQL test suite), ADR 0001 (formalizing the `qr_codes` RLS boundary ahead of Step 4), and Step 4 **in progress** — Step 4.0 (shared infrastructure: migration `0005`, idempotency, accessible-branch resolution), Step 4.1 (**Restaurant CRUD**), Step 4.2 (**Branch CRUD**, including the Address relationship), and Step 4.3 (**Operating Hours**, full-week replace nested into `GET /branches/{id}`) all complete — see §21 for the full record. Everything in §1–§20 below predates Sprint 5 and describes Sprint 4.1 (Tenant Platform), which is unchanged and still merged into `develop`.

---

## 1. Current Sprint

**Sprint 5 — Restaurant Platform** (the second business platform / bounded context). Step 1 (architecture planning), Step 2 (RBAC Foundation), Step 3 (Restaurant Platform Data Layer), and ADR 0001 are complete. **Step 4 (Application/API layer) is in progress, gated sub-step by sub-step**: Step 4.0 (shared infrastructure), Step 4.1 (Restaurant CRUD), Step 4.2 (Branch CRUD), and Step 4.3 (Operating Hours) are all complete and reported; Step 4.4 has not begun. See §21. Frontend and business workflows (POS/Billing/QR Ordering/KDS/Inventory/Liquor/Online Ordering/Loyalty/Expenses/Attendance/Owner Dashboard) have **not** been started.
...
Per the user's instruction: **Step 4.3 is complete and reported; Step 4.4 does not begin without further direction.** No Table Zone/Table/QR/Menu/Modifiers/Pricing/Availability/Orders/POS/Kitchen/Inventory CRUD, no frontend, no business workflow exists yet. This branch is not merged into `develop` and not pushed to GitHub.

---

## Engineering Status

**Completed Sprints:** 0 (Product Blueprint), 1 (Technical Architecture v1.0), 1.5 (TAD remediation → v2.0), 2 (Data Architecture v1.0), 2.6 (Data Architecture remediation → v2.0), 3 (Identity Platform backend), **4.1 (Tenant Platform, all 5 steps — merged into `develop`)**. **Sprint 5 (Restaurant Platform)** Steps 1–3 complete, ADR 0001 complete, **Step 4 in progress** (4.0, 4.1, 4.2, and 4.3 of 12 planned sub-steps complete) — on `feature/restaurant-platform`, not yet merged; see §21.

**In progress:** Sprint 5 Step 4 (Application/API layer), proceeding one gated sub-step at a time. Sprint 4.1 is merged into `develop` (merge commit `80fcb9d`, pushed to `origin/develop`) and unaffected by Sprint 5. Sprint 5's `feature/restaurant-platform` branch (38 commits ahead of `develop` as of `6cbd4f3`, plus this documentation commit, §21) has Step 4.0 (shared infrastructure), Step 4.1 (Restaurant CRUD), Step 4.2 (Branch CRUD), and Step 4.3 (Operating Hours) all complete and reported — not merged, not pushed, awaiting direction before Step 4.4 begins. Below this point (commit list, "Current Branch/PR/Milestone/Feature/Module") describes `develop`'s own state as of the Sprint 4.1 merge and is unchanged by Sprint 5 — see §21 for Sprint 5's own branch/commit/status details.
