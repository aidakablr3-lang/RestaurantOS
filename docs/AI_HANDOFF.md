...
**Known limitation requiring a documentation-only decision (not a code defect, no code changed for this):** the architecture doc has an internal tension between §3.1's `QRCode` entry ("RLS: Branch-scoped for the *management* read path... the *resolution* read path is... an explicit, narrow exception") and §14's blanket migration-strategy statement ("every new table gets `ENABLE ROW LEVEL SECURITY`"). §14 is Sprint 5 Step 1's early planning-stage summary (predates the specific resolution-path conflict being worked through); §3.1 is the detailed, entity-specific spec that explicitly anticipated an exception for this one table. A real RLS policy structurally cannot distinguish "the trusted resolution code path" from any other query — Postgres RLS sees only column values and session settings — so there is no way to keep a standard RLS policy on `qr_codes` and have the (not-yet-built) unauthenticated resolution endpoint work at all. The implemented decision (no RLS on `qr_codes` at all, application-layer tenant scoping for the management path) is broader than §3.1's literal text (which describes RLS staying on for the management path specifically, with only the resolution path exempted — e.g. via a distinct, `BYPASSRLS`-capable connection role) but is the only approach that doesn't invent a new isolation mechanism (architecture §4.2 forbids a second one) or add infrastructure (a privileged DB role/connection) that belongs to Step 4 (the resolution endpoint doesn't exist yet). Not changed again this pass per the explicit instruction to leave it unless the architecture *unambiguously* requires something else — it doesn't; it's internally inconsistent. Flagged here for the user's awareness, not silently resolved either way.

### Explicit stop point

Per the user's instruction: **do not proceed to Restaurant Platform Step 4 (REST API) without explicit approval.** No Restaurant Platform REST API, frontend, or business workflow exists yet. This branch is not merged into `develop` and not pushed to GitHub.
