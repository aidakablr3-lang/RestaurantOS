...
**Updated by:** Sprint 5 Step 1 (Restaurant Platform architecture planning, documentation-only), Step 2 (RBAC Foundation: architecture doc + 8-commit implementation, plus a user-directed **Commit 9 correction pass** fixing a real router-level branch-scoped-authorization gap), and Step 3 (**Restaurant Platform Data Layer**: 15-table migration `0004`, domain model, repositories, and a real-PostgreSQL test suite — see §21 for the full record). Everything in §1–§20 below predates Sprint 5 and describes Sprint 4.1 (Tenant Platform), which is unchanged and still merged into `develop`.

---

## 1. Current Sprint

**Sprint 5 — Restaurant Platform** (the second business platform / bounded context). Step 1 (architecture planning), Step 2 (RBAC Foundation, a prerequisite Step 1's own review identified), and Step 3 (Restaurant Platform Data Layer — schema, domain model, repositories, tests; no API, no frontend, no business workflows) are complete — see §21. Restaurant Platform business entities' REST API, frontend, and business workflows (POS/Billing/QR Ordering/KDS/Inventory/Liquor/Online Ordering/Loyalty/Expenses/Attendance/Owner Dashboard) have **not** been started; explicit STOP after Step 3 per the user's instruction, awaiting approval before Step 4.
...
Per the user's instruction: **do not begin Step 4 implementation without a separately-approved plan.** A Step 4 (Application/API layer) implementation plan was requested and produced for review, covering Restaurant/Branch CRUD, Table Zone/Table management, Operating Hours, Menu Categories/Items/branch pricing, Modifier Groups/Modifiers, Availability, Reservations, the QR-code resolution boundary (per ADR 0001's contract), RBAC/tenant/branch authorization, idempotency, pagination/filtering, validation, error handling, and Outbox/domain-event integration — but **no Step 4 code has been written**. No Restaurant Platform REST API, frontend, or business workflow exists yet. This branch is not merged into `develop` and not pushed to GitHub.

---

## Engineering Status

**Completed Sprints:** 0 (Product Blueprint), 1 (Technical Architecture v1.0), 1.5 (TAD remediation → v2.0), 2 (Data Architecture v1.0), 2.6 (Data Architecture remediation → v2.0), 3 (Identity Platform backend), **4.1 (Tenant Platform, all 5 steps — merged into `develop`)**, **5 Steps 1–3 (Restaurant Platform architecture + RBAC Foundation + Restaurant Platform Data Layer — on `feature/restaurant-platform`, not yet merged; see §21)**.

**In progress:** Nothing actively in flight. Sprint 4.1 is merged into `develop` (merge commit `80fcb9d`, pushed to `origin/develop`) and unaffected by Sprint 5. Sprint 5's `feature/restaurant-platform` branch (18 commits as of `08a8ed5`, plus this documentation commit, §21) is complete for its approved scope through Step 3 — including a user-directed RBAC correction pass and the Restaurant Platform data layer (schema, domain model, repositories, tests) — not merged, not pushed, awaiting explicit approval before Restaurant Platform Step 4 (REST API) begins. Below this point (commit list, "Current Branch/PR/Milestone/Feature/Module") describes `develop`'s own state as of the Sprint 4.1 merge and is unchanged by Sprint 5 — see §21 for Sprint 5's own branch/commit/status details.
